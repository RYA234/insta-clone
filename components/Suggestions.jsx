    ...
    useEffect(() =>{
        (async () => {
            const randomUsersResponse = await fetch(
                "https://randomuser.me/api/?results=5&inc=name,login,picture,location"
              ).then((res) => res.json());
              setSuggestions(randomUsersResponse.results)
            })();    
        },[]);
    return(
            ...
            {suggestions.map(suggestion => (
                <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex-1 ml-4">
                        <h2 className="font-semibold text-sm">{suggestion.login.username}</h2>
                        <h3 className="text-sm text-gray-400 truncate w-[230px]">{suggestion.location.country}</h3>
                    </div>
